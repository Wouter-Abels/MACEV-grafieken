...
## Load data ##
with open ('assets/geodata.json', 'r') as f:
    geodata = json.load(f)
all_data = pd.read_csv('assets/historic_and_data_n.csv', dtype={'externalreference': str})
...
def graph_total_update(dropdown_value):
    ...
    fig1 = px.bar(total_plot_data.apply(lambda x: x*100/sum(x),axis=1),
        color_discrete_map=graphs.macev_taxongroup_colours,
